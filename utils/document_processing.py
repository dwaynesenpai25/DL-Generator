    ...
    try:
        ...
        with TemporaryDirectory() as global_temp_dir_str:
            ...
            if output_format == "zip":
                ...
                logger.debug(f"Created ZIP file: {final_zip_path_on_server}")
            ...
            for final_area, group_df in valid_rows.groupby('FINAL_AREA'):
                ...
                if session_state.get('selected_mode') in ["DL Only", "DL w/ Transmittal"]:
                    ...
                    for record_index, (_, row) in enumerate(group_df.iterrows()):
                        ...
                        yield json.dumps({
                            ...
                            'message': f"📝 Generating DL document {record_index + 1}/{area_records} for {row.get('LEADS_CHNAME', 'Unknown')} in {final_area}",
                        ...
                        if filled_doc_obj:
                            ...
                            dl_docx_files.append(str(docx_output_path))
                            total_docx_files += 1
                    ...
                    if not temp_transmittal_path:
                        ...
                        if transmittal_docs_list:
                            for doc_idx, transmittal_doc_obj in enumerate(transmittal_docs_list):
                                ...
                                transmittal_docx_files.append(str(transmittal_docx_output_path))
                ...
                if output_format == "zip" and (dl_docx_files or transmittal_docx_files):
                    yield json.dumps({
                        'progress': 75 + (current_area_index / total_areas) * 5,
                        'message': f"🔄 Preparing PDF conversion for {len(dl_docx_files)} DL and {len(transmittal_docx_files)} transmittal files in {final_area}",
                        'stage': 'pdf_conversion_start',
                        'conversion_details': {
                            'area': final_area,
                            'files_to_convert': len(dl_docx_files) + len(transmittal_docx_files),
                            'estimated_batches': (len(dl_docx_files) + len(transmittal_docx_files) + 299) // 300
                        }
                    }) + '\n'

                    # Convert DL and transmittal DOCX files to PDFs
                    all_docx_files = dl_docx_files + transmittal_docx_files
                    pdf_files_for_area = []
                    ...
                    if all_docx_files:
                        ...
                        yield json.dumps({
                            'progress': 80,
                            'message': f"🔄 Starting PDF conversion for {len(all_docx_files)} files in {total_batches} batches",
                            'stage': 'conversion',
                            'conversion_details': {
                        ...
                        # Separate DL and transmittal PDFs
                        dl_pdfs = [pdf for pdf in pdf_files_for_area if 'dl_' in Path(pdf).name]
                        transmittal_pdfs = [pdf for pdf in pdf_files_for_area if 'transmittal_' in Path(pdf).name]
                        logger.debug(f"Area {final_area}: {len(dl_pdfs)} DL PDFs, {len(transmittal_pdfs)} transmittal PDFs")

                        # Merge DL PDFs
                        dl_merged_pdf_path = None
                        if dl_pdfs and session_state.get('selected_mode') in ["DL Only", "DL w/ Transmittal"]:
                            dl_merged_pdf_path = area_output_dir / f"dl_{final_area}.pdf"
                            merger = PdfMerger()
                            try:
                                for pdf in sorted(dl_pdfs):  # Sort for consistent order
                                    if Path(pdf).exists():
                                        merger.append(pdf)
                                        logger.debug(f"Added {pdf} to DL merger")
                                    else:
                                        logger.error(f"DL PDF not found for merging: {pdf}")
                                merger.write(str(dl_merged_pdf_path))
                                merger.close()
                                logger.info(f"Merged {len(dl_pdfs)} DL PDFs into {dl_merged_pdf_path}")
                            except Exception as e:
                                logger.error(f"Failed to merge DL PDFs for {final_area}: {e}\n{traceback.format_exc()}")
                                merger.close()
                                dl_merged_pdf_path = None

                        # Merge transmittal PDFs
                        transmittal_merged_pdf_path = None
                        if transmittal_pdfs and session_state.get('selected_mode') in ["DL w/ Transmittal", "Transmittal Only"]:
                            transmittal_merged_pdf_path = area_output_dir / f"transmittal_{final_area}.pdf"
                            merger = PdfMerger()
                            try:
                                for pdf in sorted(transmittal_pdfs):  # Sort for consistent order
                                    if Path(pdf).exists():
                                        merger.append(pdf)
                                        logger.debug(f"Added {pdf} to transmittal merger")
                                    else:
                                        logger.error(f"Transmittal PDF not found for merging: {pdf}")
                                merger.write(str(transmittal_merged_pdf_path))
                                merger.close()
                                logger.info(f"Merged {len(transmittal_pdfs)} transmittal PDFs into {transmittal_merged_pdf_path}")
                            except Exception as e:
                                logger.error(f"Failed to merge transmittal PDFs for {final_area}: {e}\n{traceback.format_exc()}")
                                merger.close()
                                transmittal_merged_pdf_path = None

                        # Add merged PDFs to ZIP
                        if zipf:
                            merged_pdfs = [p for p in [dl_merged_pdf_path, transmittal_merged_pdf_path] if p and p.exists()]
                            logger.debug(f"Adding {len(merged_pdfs)} merged PDFs to ZIP for area {final_area}")
                            for pdf in merged_pdfs:
                                try:
                                    zipf.write(str(pdf), f"{final_area}/{pdf.name}")
                                    logger.debug(f"Added merged PDF {pdf} to ZIP")
                                except Exception as e:
                                    logger.error(f"Failed to add merged PDF {pdf} to ZIP: {e}\n{traceback.format_exc()}")

                        # Clean up individual PDFs (optional, to save disk space)
                        for pdf in pdf_files_for_area:
                            try:
                                if Path(pdf).exists():
                                    os.remove(pdf)
                                    logger.debug(f"Deleted individual PDF {pdf}")
                            except Exception as e:
                                logger.warning(f"Failed to delete individual PDF {pdf}: {e}")

                        # Send conversion and merging summary
                        total_success_rate = (len(pdf_files_for_area) / len(all_docx_files)) * 100 if all_docx_files else 0
                        status_icon = "🎉" if total_success_rate > 95 else "✅" if total_success_rate > 80 else "⚠️"
                        
                        yield json.dumps({
                            'progress': 95,
                            'message': f"{status_icon} PDF conversion and merging complete for {final_area}: {len(dl_pdfs)} DL, {len(transmittal_pdfs)} transmittal files ({total_success_rate:.1f}% success)",
                            'stage': 'conversion_complete',
                            'conversion_summary': {
                                'total_files': len(all_docx_files),
                                'total_batches': total_batches,
                                'successful': len(pdf_files_for_area),
                                'failed': len(all_docx_files) - len(pdf_files_for_area),
                                'success_rate': total_success_rate,
                                'total_time': "N/A",
                                'conversion_rate': 0,
                                'batches_processed': total_batches,
                                'dl_merged': str(dl_merged_pdf_path) if dl_merged_pdf_path else "None",
                                'transmittal_merged': str(transmittal_merged_pdf_path) if transmittal_merged_pdf_path else "None"
                            }
                        }) + '\n'
                
                elif output_format == "print" and (dl_docx_files or transmittal_docx_files):
                    area_docx_files_for_print[final_area] = dl_docx_files + transmittal_docx_files
                    yield json.dumps({
                        'progress': (current_area_index / total_areas) * 100,
                        'message': f"📄 Prepared {len(dl_docx_files)} DL and {len(transmittal_docx_files)} transmittal DOCX files for printing in {final_area}",
                        'stage': 'print_preparation',
                        'print_details': {
                            'area': final_area,
                            'files_prepared': len(dl_docx_files) + len(transmittal_docx_files)
                        }
                    }) + '\n'
        ...
        logger.error(f"Outer error in generate_pdfs_stream for user {user_email}: {e}\n{traceback.format_exc()}")
        yield json.dumps({
