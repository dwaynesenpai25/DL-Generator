...
# Create output and barcode directories with user-specific subdirectories
...
def reset_user_session_state(user_email: str):
    ...
    with SESSION_LOCK:
        if user_email in USER_SESSIONS:
            ...
            # if session_state.get('user_output_dir'):
            cleanup_user_directory(OUTPUT_DIR)
...
async def set_mode(request: ModeRequest, user_info: dict = Depends(get_current_user)):
    ...
    if session_state.get('selected_mode') and session_state.get('selected_mode') != request.mode:
        logger.info(f"Mode changing from {session_state.get('selected_mode')} to {request.mode} for user {user_email}, resetting state")
        reset_user_session_state(user_email)
        session_state = get_user_session_state(user_email)
    ...
    if request.mode in ["DL w/ Transmittal", "Transmittal Only"]:
        try:
            ...
            logger.error(f"Failed to set mode {request.mode} for user {user_email}: {e}")
...
async def cleanup(user_info: dict = Depends(get_current_user)):
    try:
        user_email = user_info.get("email", "")
        # Enhanced cleanup that resets user-specific session
        reset_user_session_state(user_email)
