...
def replace_in_text_boxes(doc, find_str, replace_with_image_path, word_app, temp_doc_path):
    try:
        # Ensure Word is in a good state
        if not word_app:
        ...
        # Attempt to recover by restarting Word
        try:
...
# Modified fill_template to integrate with Word instance
def fill_template(doc, mapping, barcode_buffer=None, signature_img_path=None, word_app=None):
    """
    Fill placeholders in the document and handle shape replacements.
    """
    ...
    # Save the document temporarily for shape replacement
    if signature_img_path and os.path.exists(signature_img_path) and "«IMAGE_SIGNATURE»" in mapping:
        with TemporaryDirectory() as temp_dir:
            ...
            temp_doc_path = replace_in_text_boxes(doc, "«IMAGE_SIGNATURE»", signature_img_path, word_app, temp_doc_path)
            doc = Document(temp_doc_path)

    return doc
# Batch convert DOCX files to PDF
def batch_convert_to_pdf(docx_files, output_dir, word_app):
    """
    Convert a list of DOCX files to PDF using a single Word instance.
    ...
    try:
        for docx_path in docx_files:
            ...
            try:
                word_doc = word_app.Documents.Open(str(os.path.abspath(docx_path)))
                word_doc.SaveAs(str(os.path.abspath(output_path)), FileFormat=17)
                word_doc.Close(SaveChanges=False)
                ...
                st.warning(f"Failed to convert {docx_path} to PDF: {e}")
                logger.error(f"Failed to convert {docx_path} to PDF: {e}")
        return pdf_files
    except Exception as e:
        st.error(f"Failed to convert DOCX files to PDF: {e}")
        logger.error(f"Failed to convert DOCX files to PDF: {e}")
        return pdf_files
...
if template_path and header_footer_template_path and base_template:
    ...
    if uploaded_excel:
        ...
        if st.button("🔄 Generate PDFs by FINAL_AREA"):
            ...
            # Initialize a single Word application instance
            ...
            try:
                ...
                for final_area, group_df in valid_rows.groupby('FINAL_AREA'):
                    # Start a new Word instance for each FINAL_AREA
                    if word_app:
                        try:
                            word_app.Quit()
                        except:
                    ...
                    for _, row in group_df.iterrows():
                        ...
                        with TemporaryDirectory() as temp_dir:
                            ...
                            filled_doc = fill_template(filled_doc, mapping, barcode_buffer, signature_img_path, word_app)
                            if not filled_doc:
                    ...
                    pdf_files = batch_convert_to_pdf(docx_files, OUTPUT_DIR, word_app)
                    ...
                    # Close Word instance after each FINAL_AREA
                    try:
                        word_app.Quit()
                    except:
                        pass
